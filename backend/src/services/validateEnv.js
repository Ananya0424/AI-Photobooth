...
const validateEnvironment = () => {
  const requiredVars = [
    ...
    { key: 'CLOUDINARY_CLOUD_NAME', service: 'Cloudinary (CDN)', pattern: /^[a-z0-9]+$/ },
    { key: 'CLOUDINARY_API_KEY', service: 'Cloudinary (CDN)', pattern: /^[0-9]+$/ },
    { key: 'CLOUDINARY_API_SECRET', service: 'Cloudinary (CDN)', pattern: /^[a-z0-9]+$/i },
  ];
  ...
  if (!emailConfigured) {
    console.log('⚠️  Email service not configured (optional — portraits will still generate).\n');
  } else {
 ...
 * @param {object} services - Object with service check functions
