...
const sendPortraitEmail = async (toEmail, userName, imageUrl) => {
  const logPrefix = `[EmailService:Nodemailer] ${toEmail}`;

  try {
    const transporter = getTransporter();

    if (!transporter) {
      console.log(`${logPrefix} Nodemailer credentials not configured.`);
      return { success: false, error: "Email credentials not configured", mock: true };
    }
    ...
    console.log(`${logPrefix} Email sent. messageId=${result.messageId}`);
...
const checkEmailHealth = async () => {
  ...
  if (!emailUser || !emailPass) {
    return {
      healthy: true,
      configured: false,
      message: "Email credentials not configured (EMAIL_USER and EMAIL_PASS missing)",
    };
  }
